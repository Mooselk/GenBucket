    ...
    public Bucket matchBucket(ItemStack item) {
        for (Map.Entry<String, Bucket> entry : buckets.entrySet()) {
            ...
            if (otherItem.getType() == item.getType() 
            		&& ((item.hasItemMeta() 
            		&& otherItem.getItemMeta().hasDisplayName()) 
            		|| (!item.hasItemMeta() 
            		&& !otherItem.getItemMeta().hasDisplayName()))
                    && otherItem.getItemMeta().getDisplayName().equals(item.getItemMeta().getDisplayName()) 
                    && ((!otherItem.getItemMeta().hasLore() && !item.getItemMeta().hasLore()) 
                    || (otherItem.getItemMeta().getLore().containsAll(item.getItemMeta().getLore())))) {
                return bucket;
            }
        }
        return null;
